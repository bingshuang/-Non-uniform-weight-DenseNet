...
class DenseNet(nn.Module):
    def __init__(
        ...
        self.se1 = SEBlock(n_channels) # add se
        ...
        n_channels = n_out_channels
        self.se2 = SEBlock(n_channels) # add se
        ...
        self.se3 = SEBlock(self.out_channels)
    ...
    def forward(self, x, x_mask):
        # print("input:",x.shape, x_mask.shape) # input: torch.Size([1, 1, 54, 53]) torch.Size([1, 54, 53])
        out = self.conv1(x)
        out = self.norm1(out)
        ...
        out = self.se1(out)
        out_mask = out_mask[:, 0::2, 0::2]
        # print("out:",out.shape, out_mask.shape)
        out = self.dense2(out)
        out = self.trans2(out)
        out = self.se2(out)
        out_mask = out_mask[:, 0::2, 0::2]
        ...
        # print("out:", out.shape, out_mask.shape) # out: torch.Size([1, 684, 4, 4]) torch.Size([1, 4, 4])
        return out, out_mask
